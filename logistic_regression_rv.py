...
# Cargando el dataset
dataset = pd.read_csv('framingham.csv')  # Lee el archivo CSV con los datos
dataset = dataset.dropna()
X = dataset[['age','sysBP']].values  # Extrae las características (Edad, Salario Estimado)
y = dataset['TenYearCHD'].values # Extrae la variable objetivo (Compra o No Compra)
...
resultado = classifier.predict(sc.transform(nuevo_paciente))
print(f"¿Tiene riesgo de CHD en 10 años? {'Sí' if resultado[0] == 1 else 'No'}")
